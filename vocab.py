...
class VocabEntry(object):
    ...
    def words2indices_complete_decode(self, sents_src, exist_oovs=None):
        ...
        for sent_src in sents_src:
            bias = 0
            if exist_oovs is not None:
                exist_oov_s = exist_oovs[idx]
                bias = len(exist_oov_s)
            word_oov_s = []
            word_src_complete_id_s = []
            for w in sent_src:
                if w not in self.word2id:
                    if exist_oovs is not None and w in exist_oov_s:
                        word_src_complete_id_s.append(vocab_size + exist_oov_s.index(w))
                        continue
                    if w not in word_oov_s:
                        word_oov_s.append(w)
                    word_src_complete_id_s.append(vocab_size + word_oov_s.index(w) + bias)
                else:
                    word_src_complete_id_s.append(self.word2id[w])
            word_src_complete_ids.append(word_src_complete_id_s)
            if exist_oovs is not None:
                word_oov_s = exist_oov_s + word_oov_s
    ...
    def words2indices_complete(self, sents_src, sents_tgt, exist_oovs=None):
        assert(len(sents_src) == len(sents_tgt))
        #word_src_ids = [[self[w] for w in s] for s in sents_src]
    ...
    def to_input_tensor_tgt_decode(self, sents_src, device, exist_oovs=None):
        (word_src_complete_ids, word_oovs, max_oov_num) = self.words2indices_complete_decode(sents_src, exist_oovs)
    ...
    def to_input_tensor_tgt(self, sents_src, sents_tgt, device, exist_oovs=None):
        (word_tgt_ids, word_src_complete_ids, word_tgt_complete_ids,
                word_oovs, max_oov_num) = self.words2indices_complete(sents_src, sents_tgt, exist_oovs)
        sents_t_src_complete = input_transpose(word_src_complete_ids, self['<pad>'])
...
if __name__ == '__main__':
    ...
    words_tgt = []
    for line in fin_src:
        words_src.append(line.strip().split()[0])
    for line in fin_tgt:
        words_tgt.append(line.strip().split()[0])

    #vocab = Vocab.build(chars)
    #print('generated vocabulary %d characters' % len(vocab.char))

    #vocab.save(args['WORD_FILE'])
    #print('vocabulary saved to %s' % args['WORD_FILE'])
